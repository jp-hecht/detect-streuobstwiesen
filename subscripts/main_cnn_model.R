...
mcc <- function(y_true,y_pred,smooth = 1.0){
   y_true_f <- k_flatten(y_true)
   y_pred_f <- k_flatten(y_pred)
   ...
   up <- (tp * tn - fp * fn)+ smooth
   down <- (k_sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn)))+ smooth
...
# Dice coefficient/F1 score --> other method for implementation
